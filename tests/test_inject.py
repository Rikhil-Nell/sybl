...
from navi.config.models import InjectConfig, NaviConfig
...
async def test_inject_sets_pastes_and_restores_clipboard(
    ...
    with (
        patch(
            "navi.inject.clipboard_win.read_clipboard_safe",
            return_value="prior text",
        ) as mock_read,
        patch("navi.inject.clipboard_win.set_clipboard") as mock_set,
        patch("navi.inject.clipboard_win.restore_focus") as mock_focus,
        patch("navi.inject.clipboard_win.simulate_paste") as mock_paste,
    ):
...
async def test_inject_skips_restore_when_disabled() -> None:
    ...
    with (
        patch("navi.inject.clipboard_win.read_clipboard_safe") as mock_read,
        patch("navi.inject.clipboard_win.set_clipboard") as mock_set,
        patch("navi.inject.clipboard_win.restore_focus"),
        patch("navi.inject.clipboard_win.simulate_paste"),
    ):
...
async def test_inject_wraps_unexpected_errors(injector: ClipboardPasteInjector) -> None:
    with (
        patch("navi.inject.clipboard_win.read_clipboard_safe", return_value=None),
        patch(
            "navi.inject.clipboard_win.set_clipboard",
...
def test_create_injector_on_windows() -> None:
    config = NaviConfig()
    with patch("navi.inject.base.sys.platform", "win32"):
        from navi.inject.base import create_injector
...
def test_create_injector_rejects_non_windows() -> None:
    config = NaviConfig()
    with patch("navi.inject.base.sys.platform", "linux"):
        from navi.inject.base import create_injector
...
def test_read_clipboard_safe_returns_skip_sentinel_on_oserror() -> None:
    with patch(
        "navi.inject.clipboard_win.read_clipboard",
        side_effect=OSError("access violation"),
    ):
        from navi.inject.clipboard_win import _SKIP_RESTORE, read_clipboard_safe

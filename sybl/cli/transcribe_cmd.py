"""`sybl transcribe` — record and transcribe via BYOK STT."""
...
from sybl.audio import AudioCaptureSession, AudioError
from sybl.cli.meter import meter_loop, wait_for_enter
from sybl.config import ConfigManager, log_path
from sybl.core.postprocess import process_text
from sybl.core.transcribe import transcribe_pcm, transcribe_stream
from sybl.logging import setup_logging
from sybl.providers import STTError, resolve_provider
...
def transcribe_command(
    ...
    sybl_config = config_manager.load()
    setup_logging(sybl_config.logging, log_file=log_path(), verbose=verbose)

    try:
        asyncio.run(_transcribe(sybl_config, seconds, device, provider, stream))
...
async def _transcribe(
    ...
    audio_config = sybl_config.audio.model_copy(
        update={"device": device if device is not None else sybl_config.audio.device}
    )
    ...
    final_text = process_text(sybl_config.postprocess, outcome.text)
